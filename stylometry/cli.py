...
from .loaders import chunk_doc, load_corpus, safe_read_text
from .metrics import (
...
from .utils import ensure_dir, maybe_make_plots, utc_now_iso, write_json
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Stylometric artifact extraction + comparison (offline).")
    parser.add_argument(
        "--task",
        choices=["normalize", "characterize", "compare", "profile_build", "anomaly_scan"],
    ...
    # Delta
    parser.add_argument(
        "--delta-mfw", type=int, default=150, help="Number of Top Most Frequent Words for Burrows' Delta."
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Enable heavy linguistic feature extraction via StyloMetrix.",
    )
    ...
    if args.task == "normalize" or args.normalize_before_analysis:
        ...
        from .loaders import split_sentences, tokenize_words
    ...
    def process_items(items, desc):
        if not items:
            return []
        rows = []
        payloads = [(i.word_tokens, i.sentences, i.text, function_words, args.mattr_window) for i in items]
        ...
        for i, m in zip(items, results):
            ...
            if hasattr(i, "path"):
                row["path"] = str(i.path)
            if hasattr(i, "chunk_id"):
                row["chunk_id"] = str(i.chunk_id)
            row.update(m)
            rows.append(row)
        return rows
    ...
    for corp in corpus_order:
        try:
            ...
            summary["corpora"][corp] = {
                ...
                "total_words_docs": (
                    int(sub_doc["word_count"].sum()) if not sub_doc.empty and "word_count" in sub_doc else 0
                ),
                ...
                "avg_sentence_len": (
                    float(sub_doc["avg_sentence_len"].mean())
                    if not sub_doc.empty and "avg_sentence_len" in sub_doc
                    else 0.0
                ),
                "avg_hapax_rate": (
                    float(sub_doc["hapax_rate"].mean()) if not sub_doc.empty and "hapax_rate" in sub_doc else 0.0
                ),
                "avg_commas_per_1000w": (
                    float(sub_doc["commas_per_1000w"].mean())
                    if not sub_doc.empty and "commas_per_1000w" in sub_doc
                    else 0.0
                ),
            }
        except Exception as e:
    ...
    if len(set([d.corpus for d in all_docs])) >= 2 and len(all_chunks) >= 4:
        ...
        if not sim_df.empty:
            ...
            write_json(
                out_dir / "ResultBundle_Comparator.json",
                {
    ...
    report_path = generate_report(
        out_dir, run_id, utc_now_iso(), summary, plot_files, ai_insights=ai_insights, doc_df=doc_df, chunk_df=chunk_df
    )
