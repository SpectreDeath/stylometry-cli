...
def get_embedding(text: str, api_base: str, model: str) -> Optional[np.ndarray]:
    ...
    try:
        ...
        print(
            f"Embedding request failed (HTP {he.code}): {he.reason}. Check if '{model}' supports embeddings and if the server has them enabled."
        )
        return None
...
def calculate_semantic_similarity_matrix(
    docs: List[DocRecord], api_base: str, model: str
