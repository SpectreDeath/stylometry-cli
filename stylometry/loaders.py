...
from .normalizer import NormalizerConfig, normalize_text_advanced
...
def load_corpus(
    label: str, path_str: str, norm_cfg: Optional[NormalizerConfig] = None
) -> Tuple[List[DocRecord], Optional[Path]]:
    temp_root = None
    docs: List[DocRecord] = []
    ...
    if path_str.startswith(("http://", "https://")):
        ...
        docs.append(
            DocRecord(corpus=label, doc_id=doc_id, path=path_str, text=text, word_tokens=tokens, sentences=sents)
        )
        return docs, None
    ...
    def _process_file(fp: Path, doc_id: str):
        ...
        sents = split_sentences(text)
        docs.append(
            DocRecord(corpus=label, doc_id=doc_id, path=str(fp), text=text, word_tokens=tokens, sentences=sents)
        )
...
def chunk_doc(doc: DocRecord, chunk_words: int, overlap: int = 0) -> List[ChunkRecord]:
    ...
    for i in range(0, len(tokens), step):
        ...
        chunks.append(
            ChunkRecord(
                corpus=doc.corpus, doc_id=doc.doc_id, chunk_id=chunk_id, text=chunk_text, word_tokens=w, sentences=sents
            )
        )
